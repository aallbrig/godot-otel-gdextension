...
void OtelTracer::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("start_span_with_parent", "name", "parent"),
                         &OtelTracer::start_span_with_parent);
}

void OtelTracer::set_internal_tracer(std::shared_ptr<opentelemetry::v1::trace::Tracer> tracer) {
...
Ref<OtelSpan> OtelTracer::start_span_with_parent(const String& name, Ref<OtelSpan> parent) {
    Ref<OtelSpan> span_wrapper;
    span_wrapper.instantiate();
    if (!_tracer) return span_wrapper;
