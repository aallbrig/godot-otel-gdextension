...
void OtelMeter::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("create_int_counter", "name", "description", "unit"),
        &OtelMeter::create_int_counter);
    ClassDB::bind_method(
        D_METHOD("create_double_histogram", "name", "description", "unit"),
        &OtelMeter::create_double_histogram);
}

void OtelMeter::set_internal_meter(
    std::shared_ptr<opentelemetry::v1::metrics::Meter> meter) {
    _meter = std::move(meter);
}

Ref<OtelCounter> OtelMeter::create_int_counter(
...
Ref<OtelHistogram> OtelMeter::create_double_histogram(
    const String& name, const String& description, const String& unit) {
